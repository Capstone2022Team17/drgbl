...
class HBMReadAndWriteSM(Module, AutoCSR):
    ...
    def __init__(self, axi_port):
        ...
        self.strb_readwrite = CSRStorage(axi_port.data_width //8, description="Indicates the byte lanes that hold valid data")
        self.address_readwrite = CSRStorage(axi_port.address_width, description="Address to perform read or write at")
        self.data_writein = CSRStorage(axi_port.data_width, description="Data to write after performing write")
        self.data_readout = CSRStatus(axi_port.data_width, description="Data to read after performing read")
        self.exec_read_done = CSRStatus(ONE_BIT_WIDE, description="High if done performing read")
        self.exec_write_done = CSRStatus(ONE_BIT_WIDE, description="High if done performing write")
        self.acknowledge_readwrite = CSRStorage(ONE_BIT_WIDE, description="Acknowledge to state machine read or write happened")
        ...
        hbm_port_fsm.act("WAIT_INSTRUCTION",
            If(self.perform_read.storage,
                NextState("PERFORM_READ_COMMAND"),
            ).Elif(self.perform_write.storage,
                NextState("PREPARE_WRITE_COMMAND"),
            ).Else(
                NextState("WAIT_INSTRUCTION"),
            )
        )
        hbm_port_fsm.act("PREPARE_WRITE_COMMAND",
            ...
            If(axi_port.aw.ready,
                NextState("PREPARE_WRITE"),
            ).Else(
                NextState("PREPARE_WRITE_COMMAND"),
            )
        )
        hbm_port_fsm.act("PREPARE_WRITE",
            axi_port.w.data.eq(self.data_writein.storage),
            axi_port.w.strb.eq(self.strb_readwrite.storage),
        ...
        hbm_port_fsm.act("PREPARE_W_RESPONSE",
            axi_port.w.data.eq(self.data_writein.storage),
            axi_port.aw.addr.eq(self.address_readwrite.storage),
        ...
        hbm_port_fsm.act("PERFORM_READ_COMMAND",
...
class HBMAXILiteAccess(Module, AutoCSR):
    ...
    def __init__(self, axi_port):
        ...
        self.aw_addr = CSRStorage(axi_port.address_width, description="Write Command address")
        ...
        self.w_strb = CSRStorage(axi_port.data_width // 8, description="Write strobe: Indicates the byte lanes that hold valid data")
        ...
        self.b_resp = CSRStatus(2, description="Write response: This indicates the status of the write transaction")
        ...
        self.ar_addr = CSRStorage(axi_port.address_width, description="Read Command address")
        ...
        self.r_resp = CSRStatus(2, description="Read response: Indicates the status of the read response")
        ...
        # Comment from AXILiteInterface: present for interconnect with other cores but not used by LiteX.
        prot = 0
        ...
        axi_port.aw.qos.eq(0) # (Quality of Service) Identifier sent for each write transaction
        axi_port.aw.id.eq(0) # 
